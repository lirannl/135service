...
const getVisibleElements = (children: { main: Element[], items: { [category: string]: Element[] } },
...
function NavBar(props: {
  ...
  useEffect(() => {
    ...
    setValue(barElems.findIndex(e => e.key === currentPath));
  },
...
export const NavGenerator = ({ funcNames, ...props }) => {
  const anchorEl = useStateObj(null as Element | null);
  const selectedCategory = useStateObj('');
  const items = funcNames.unloaded ? {} : (funcNames as { func: string, category: string }[]).reduce((acc, curr) => {
  ...
  }, {} as { [category: string]: Element[] });
  useEffect(() => {
    //console.log(items);
  }, [funcNames]);
  return <NavBar {...props}>
    {{
      main: [<Tab key="" label="Home" />, <Tab onClick={event => {
      ...
      menu: <Menu
        ...
        open={!!anchorEl.value}
      >
        {funcNames.unloaded ? [] : (funcNames as { func: string, category: string }[])
