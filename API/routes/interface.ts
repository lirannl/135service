...
export const pyInterface = async function (ctx: RouterContext, next: () => Promise<void>) {
  ...
  const errString = new TextDecoder().decode(rawErr.subarray(0, outLength! - 1));
  const outputString = new TextDecoder().decode(rawOutput);
  if (code.success) {
    ctx.response.status = 200;
    ctx.response.body = { message: outputString };
  }
  else if (errString.startsWith("No such algorithm")) {
    next();
  }
  else {
    ctx.response.status = 400;
    ctx.response.body = { message: errString };
  }
}